...
> Escopo em números: ~71.700 linhas de TypeScript em `src/`, 5 sistemas de RPG,
> 57 modelos no banco, 113 testes automatizados.
...
— por isso os 113 testes rodam em milissegundos, sem banco e sem servidor.
...
113 testes em 6 arquivos, sem banco e sem servidor.
...
| Limitação | Situação |
|---|---|
| **Sem vínculo Mestre↔Jogador** | `CampaignMember` e `CampaignCharacter` existem no schema mas não são usados. O Mestre cadastra personagens manualmente; não há como puxar a ficha real de um jogador. É a maior lacuna funcional. |
| **Sem compartilhamento de ficha** | `Character.shareToken` e `isPublic` existem e não são usados. Não há link público de leitura. |
| **Tabelas de campanha duplicadas** | Aplicação unificada, banco ainda com 6 tabelas por sistema. |
| **Fichas em JSON não são consultáveis** | Trade-off da seção 3.2. |
| **Export em PDF do visual desativado** | Só o layout estruturado de D&D exporta. |
| **Acessibilidade parcial** | Poucos rótulos ARIA; alguns controles em `<div>` com `onClick`, não focáveis por teclado. |
| **Responsividade desigual** | Star Wars não tem folha de estilo responsiva própria. |
| **Estilos inline** | Mais de 6.000 objetos `style={{…}}`; Tailwind está instalado e praticamente não é usado. |
| **Componentes monolíticos** | `dnd/[id]/SheetClient.tsx` passa de 2.900 linhas. |
