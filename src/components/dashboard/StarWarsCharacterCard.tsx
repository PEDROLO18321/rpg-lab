...
export function StarWarsCharacterCard({ id, name, species, className, level, pvCurrent, pvMax, portraitUrl }: Props) {
  ...
  async function duplicate() {
    ...
    setBusy("duplicar");
    try {
      const res = await fetch(`/api/starwars/characters/${id}/duplicate`, { method: "POST" });
  ...
  async function remove() {
    ...
    setBusy("excluir");
    try {
      const res = await fetch(`/api/starwars/characters/${id}`, { method: "DELETE" });
      if (res.ok) router.refresh();
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${hovered ? ACCENT_BORD : "rgba(255,255,255,0.08)"}`,
        borderRadius: 8,
        padding: "22px 20px", cursor: "pointer",
        transition: "border-color 0.2s, transform 0.2s, box-shadow 0.2s",
        transform: hovered ? "translateY(-3px)" : "translateY(0)",
        boxShadow: hovered ? "0 12px 40px rgba(59,130,196,0.2)" : "0 2px 10px rgba(0,0,0,0.25)",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 44, height: 44, borderRadius: 6, background: ACCENT_DIM, border: `1px solid ${ACCENT_BORD}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-cinzel), serif", fontSize: "0.9rem", fontWeight: 700, color: ACCENT_LIGHT, flexShrink: 0, overflow: "hidden" }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ fontSize: "0.76rem", color: "var(--text-muted)", marginTop: 2 }}>{[species, className].filter(Boolean).join(" · ") || "Sem ficha ainda"}</p>
        </div>
      </div>
