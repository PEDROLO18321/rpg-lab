...
const ACCENT_DIM  = "rgba(125,156,62,0.14)";
const ACCENT_BORD = "rgba(125,156,62,0.32)";
...
export function InvestigatorCard({ id, name, occupation, era, age, sanCurrent, sanMax, portraitUrl }: Props) {
  ...
  const eraLabel = era === "1920s" ? "Anos 1920" : era === "modern" ? "Moderno" : null;
  ...
  async function remove() {
    ...
    setBusy("excluir");
    try {
      const res = await fetch(`/api/cthulhu/characters/${id}`, { method: "DELETE" });
      if (res.ok) router.refresh();
    } finally {
      setBusy(null);
      setMenuOpen(false);
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered
          ? `0 12px 40px rgba(125,156,62,0.12)`
    ...
    >
      ...
      >
        <button
          aria-label="Opções do investigador"
