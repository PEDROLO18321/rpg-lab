...
import { STARWARS_INCLUDE, STARWARS_STORY_FIELDS } from "@/lib/starwars/masterResources";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const campaign = await prisma.campaign.findUnique({ where: { id }, include: STARWARS_INCLUDE });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.story && typeof body.story === "object") {
    for (const f of STARWARS_STORY_FIELDS) if (typeof body.story[f] === "string") storyData[f] = body.story[f];
  }
  ...
  await prisma.campaign.update({
    ...
    data: {
      ...
      ...(Object.keys(storyData).length
        ? { starWarsStory: { upsert: { create: storyData, update: storyData } } } : {}),
    },
  });
