...
import { DND_INCLUDE, DND_STORY_FIELDS } from "@/lib/dnd/masterResources";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const campaign = await prisma.campaign.findUnique({ where: { id }, include: DND_INCLUDE });
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.story && typeof body.story === "object") {
    for (const f of DND_STORY_FIELDS) if (typeof body.story[f] === "string") storyData[f] = body.story[f];
  }
  ...
  await prisma.campaign.update({
    ...
    data: {
      ...
      ...(Object.keys(storyData).length
        ? { dndStory: { upsert: { create: storyData, update: storyData } } } : {}),
    },
  });
