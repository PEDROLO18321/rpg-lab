...
import type { CthulhuCampaign, CthulhuItem } from "@/lib/cthulhu/cthulhuCampaignStorage";

type ItemType = CthulhuItem["type"];

const TYPE_OPTIONS: { value: ItemType; label: string }[] = [
  { value: "tomo", label: "Tomo Proibido" },
  { value: "artefato", label: "Artefato do Mythos" },
...
function blankItem(): CthulhuItem {
  return { id: crypto.randomUUID(), name: "", description: "", type: "misc", mythos: false, sessionId: null };
}

interface Props { campaign: CthulhuCampaign; onChange: (c: CthulhuCampaign) => void; }

export function CthulhuArtefacts({ campaign, onChange }: Props) {
  ...
  const [newItem, setNewItem] = useState<CthulhuItem>(blankItem());
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [editingItem, setEditingItem] = useState<CthulhuItem | null>(null);
  ...
  function updateItem(updated: CthulhuItem) {
  ...
  function toggleExpand(item: CthulhuItem) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <select value={filterMythos} onChange={(e) => setFilterMythos(e.target.value as "todos" | "mythos" | "normal")} style={{ ...selectStyle, width: "auto", minWidth: 140 }}>
          <option value="todos">Mythos e Normal</option>
          <option value="mythos">Só do Mythos</option>
          <option value="normal">Não-Mythos</option>
        </select>
        <select value={filterSession} onChange={(e) => setFilterSession(e.target.value)} style={{ ...selectStyle, width: "auto", minWidth: 160 }}>
          <option value="todos">Todas as Origens</option>
          <option value="cenario">Geral (cenário)</option>
          {sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}
        </select>
        <div style={{ flex: 1 }} />
        <button onClick={() => { setAdding((v) => !v); setNewItem(blankItem()); }} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 18px", background: "linear-gradient(135deg, #a3b86c 0%, #7d9c3e 100%)", color: "#06090f", border: "none", borderRadius: "var(--radius)", fontSize: "0.82rem", fontWeight: 700, cursor: "pointer" }}>
      ...
      {/* Add form */}
      {adding && (
        <div style={{ background: "var(--surface)", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius-xl)", padding: "20px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <input style={inputStyle} value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} placeholder="Ex: Necronomicon, Amuleto de Cthulhu, Revólver .45" autoFocus />
            </div>
            <div>
            ...
            <div>
              ...
              <select style={selectStyle} value={newItem.sessionId ?? ""} onChange={(e) => setNewItem({ ...newItem, sessionId: e.target.value || null })}>
                <option value="">Geral (cenário)</option>
                {sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}
              </select>
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.78rem", color: "var(--text-muted)", cursor: "pointer", marginBottom: 12 }}>
                <input type="checkbox" checked={newItem.mythos} onChange={(e) => setNewItem({ ...newItem, mythos: e.target.checked })} />
                Item do Mythos (relacionado a entidades sobrenaturais)
              </label>
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={labelStyle}>Descrição / Efeitos</label>
              <textarea style={{ ...inputStyle, resize: "vertical", minHeight: 80 }} value={newItem.description} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })} placeholder="Propriedades, lore, rituais, perigo para a sanidade..." />
            </div>
          </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map((item) => {
            ...
            return (
              <div key={item.id} style={{ background: "var(--surface)", border: `1px solid ${isOpen ? "rgba(125,156,62,0.32)" : "var(--border)"}`, borderRadius: "var(--radius-xl)", overflow: "hidden", transition: "border-color 0.2s" }}>
                <button onClick={() => toggleExpand(item)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 20px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
                  ...
                  {item.mythos && (
                    <span style={{ fontSize: "0.63rem", fontWeight: 700, color: "#f87171", background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: "var(--radius-xs)", padding: "2px 7px", flexShrink: 0 }}>Mythos</span>
                  )}
                  ...
                  <span style={{ fontSize: "0.65rem", fontWeight: 700, color: "#a3b86c", background: "rgba(125,156,62,0.14)", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius-xs)", padding: "2px 7px", flexShrink: 0 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      ...
                      <div style={{ gridColumn: "1 / -1" }}>
                        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.78rem", color: "var(--text-muted)", cursor: "pointer" }}>
                          <input type="checkbox" checked={editing.mythos} onChange={(e) => updateItem({ ...editing, mythos: e.target.checked })} />
                          Item do Mythos
                        </label>
                      </div>
                      <div style={{ gridColumn: "1 / -1" }}>
