...
import { type CthulhuCampaign, type CthulhuNPC, type CthulhuNPCAttack } from "@/lib/cthulhu/cthulhuCampaignStorage";

const OCCUPATIONS = [
  "Antiquário", "Artista", "Cientista", "Clérigo", "Criminoso", "Detetive", "Diplomata",
  "Escritor", "Estudante", "Fazendeiro", "Jornalista", "Médico", "Militar", "Músico",
  "Navegador", "Ocultista", "Pescador", "Policial", "Professor", "Trabalhador",
  "Cultista", "Cultuador de Mythos", "Agente Secreto", "Louco",
];

const PERSONALITIES = [
  "Cético e racional, rejeita o sobrenatural",
  "Curioso demais para seu próprio bem",
  "Aterrorizado pelo que sabe mas não consegue esquecer",
  "Finge normalidade enquanto despedaça por dentro",
  "Fanático por um culto ou entidade",
  "Calmo sob pressão, estoicamente corajoso",
  "Nervoso e propenso a crises",
  "Pragmático, faz o que é necessário",
  "Teme a escuridão desde criança",
  "Obsessivo com teoria específica sobre o Mythos",
];

const NAMES = [
  "Herbert", "Agatha", "Randolph", "Mildred", "Edward", "Florence", "Charles",
  "Dorothea", "Walter", "Beatrice", "Howard", "Constance", "George", "Harriet",
  "Arkham", "Innsmouth", "Dunwich", "Kingsport", "Miskatonic",
  "Ibrahim", "Yusuf", "Chen", "Akira", "Pierre", "Heinrich", "Nikolai",
];

const RANDOM_ATTACKS: CthulhuNPCAttack[] = [
  { name: "Punho", skill: "Briga 50%", damage: "1d3+DB", description: "Ataque corpo-a-corpo" },
  { name: "Faca", skill: "Briga 25%", damage: "1d4+2+DB", description: "Ataque corpo-a-corpo ou arremesso" },
  ...
  { name: "Cassetete", skill: "Briga 30%", damage: "1d6+DB", description: "Ataque corpo-a-corpo" },
  { name: "Espingarda", skill: "Armas de Fogo 30%", damage: "4d6", description: "Alcance 10m" },
];

const ATTR_LABELS: { key: keyof Pick<CthulhuNPC, "str"|"con"|"siz"|"dex"|"int"|"pow"|"app"|"edu">; label: string }[] = [
  { key: "str", label: "FOR" }, { key: "con", label: "CON" }, { key: "siz", label: "TAM" },
  { key: "dex", label: "DES" }, { key: "int", label: "INT" }, { key: "pow", label: "POD" },
  { key: "app", label: "APA" }, { key: "edu", label: "EDU" },
];

function rand<T>(arr: T[]): T { return arr[Math.floor(Math.random() * arr.length)]; }
function randBetween(min: number, max: number) { return Math.floor(Math.random() * (max - min + 1)) + min; }

function emptyNpc(): Omit<CthulhuNPC, "id"> {
  return { name: "", occupation: "", age: null, gender: "", nationality: "", description: "", personality: "", mythosTies: "", notes: "", str: null, con: null, siz: null, dex: null, int: null, pow: null, app: null, edu: null, hp: null, san: null, attacks: [] };
}

function randomNpc(): Omit<CthulhuNPC, "id"> {
  const con = randBetween(30, 80);
  const siz = randBetween(40, 80);
  const hp = Math.floor((con + siz) / 10);
  const pow = randBetween(30, 80);
  return {
    name: rand(NAMES),
    occupation: rand(OCCUPATIONS),
    age: randBetween(20, 65),
    gender: rand(["Masculino", "Feminino", "Não-binário"]),
    nationality: rand(["Americano", "Britânico", "Francês", "Alemão", "Italiano", "Brasileiro"]),
    description: rand(["Olhos fundos e cansados, sempre nervoso", "Aparência comum que esconde segredos", "Roupas bem cuidadas, postura rígida", "Mãos sempre tremendo levemente", "Cicatrizes inexplicáveis no pescoço"]),
    personality: rand(PERSONALITIES),
    mythosTies: rand(["Nenhum (por enquanto)", "Encontrou algo que não deveria", "Membro de culto menor", "Sonhos recorrentes com entidades", "Possui tomo proibido"]),
    notes: "",
    str: randBetween(30, 80),
    con,
    siz,
    dex: randBetween(30, 80),
    int: randBetween(50, 90),
    pow,
    app: randBetween(30, 80),
    edu: randBetween(40, 90),
    hp,
    san: pow,
    attacks: [{ ...rand(RANDOM_ATTACKS) }],
  };
}

interface Props { campaign: CthulhuCampaign; onChange: (c: CthulhuCampaign) => void; }

export function CthulhuNpcCreator({ campaign, onChange }: Props) {
  const [mode, setMode] = useState<"random" | "manual">("random");
  const [form, setForm] = useState<Omit<CthulhuNPC, "id">>(randomNpc());
  ...
  function saveNpc() {
    ...
    const npc: CthulhuNPC = { ...form, id: crypto.randomUUID() };
    onChange({ ...campaign, npcs: [npc, ...campaign.npcs] });
    setForm(mode === "random" ? randomNpc() : emptyNpc());
  }

  function deleteNpc(id: string) {
    onChange({ ...campaign, npcs: campaign.npcs.filter((n) => n.id !== id) });
    if (expanded === id) setExpanded(null);
  }

  function addAttack() {
    if (!newAtk.name.trim()) return;
    setForm({ ...form, attacks: [...form.attacks, { ...newAtk }] });
    setNewAtk({ name: "", skill: "", damage: "", description: "" });
  }

  function removeAttack(i: number) {
    setForm({ ...form, attacks: form.attacks.filter((_, j) => j !== i) });
  }

  function addToInitiative(npc: CthulhuNPC) {
    const dex = addInitValue !== "" ? Number(addInitValue) : (npc.dex ?? 50);
    onChange({
      ...campaign,
      combatants: [
        ...campaign.combatants,
        { id: crypto.randomUUID(), name: npc.name, dex, hp: npc.hp, maxHp: npc.hp, san: npc.san, maxSan: npc.san, isPlayer: false },
      ],
    });
    setAddInitTarget(null);
    setAddInitValue("");
  }
  ...
  function textField(label: string, key: keyof typeof form, opts?: { list?: string[]; multi?: boolean }) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        {opts?.list ? (
          ...
          <textarea value={form[key] as string} onChange={(e) => setForm({ ...form, [key]: e.target.value })} rows={2} style={{ ...inputStyle, resize: "vertical" }} />
        ) : (
  ...
  function numField(label: string, key: keyof typeof form) {
    return (
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: 4, width: "fit-content" }}>
        {(["random", "manual"] as const).map((m) => (
          <button key={m} onClick={() => switchMode(m)} style={{ padding: "7px 18px", background: mode === m ? "rgba(125,156,62,0.14)" : "transparent", color: mode === m ? "#a3b86c" : "var(--text-muted)", border: mode === m ? "1px solid rgba(125,156,62,0.32)" : "1px solid transparent", borderRadius: "var(--radius-lg)", fontSize: "0.82rem", fontWeight: mode === m ? 700 : 500, cursor: "pointer" }}>
      ...
      {/* Form */}
      <div style={{ padding: "24px", background: "var(--surface)", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius-xl)", marginBottom: 28 }}>
        {/* Basic info */}
        ...
        <div style={{ marginBottom: 12 }}>
          {textField("Ligação com o Mythos", "mythosTies")}
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={labelStyle}>Notas</label>
          <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} placeholder="Segredos, motivações, conexões com investigadores..." style={{ ...inputStyle, resize: "vertical" }} />
        </div>

        {/* Stats */}
        <div style={{ borderTop: "1px solid rgba(125,156,62,0.32)", paddingTop: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 8, marginBottom: 12 }}>
            {ATTR_LABELS.map(({ key, label }) => numField(label, key))}
          </div>
        ...
        <div style={{ borderTop: "1px solid rgba(125,156,62,0.32)", paddingTop: 16, marginBottom: 16 }}>
                ...
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
                      {atk.skill && <span style={{ color: "#a3b86c" }}>{atk.skill}</span>}
                      {atk.skill && atk.damage && " · "}
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            <div style={{ flex: 1 }}>
              ...
              <input value={newAtk.description} onChange={(e) => setNewAtk({ ...newAtk, description: e.target.value })} placeholder="Ex: Alcance 15m, 6 balas" style={inputStyle} />
            </div>
            <button onClick={addAttack} disabled={!newAtk.name.trim()} style={{ padding: "8px 14px", background: newAtk.name.trim() ? "rgba(125,156,62,0.14)" : "var(--surface-2)", color: newAtk.name.trim() ? "#a3b86c" : "var(--text-subtle)", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius)", fontSize: "0.82rem", fontWeight: 700, cursor: newAtk.name.trim() ? "pointer" : "not-allowed", whiteSpace: "nowrap" }}>
              + Ataque
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={saveNpc} disabled={!form.name.trim()} style={{ padding: "10px 22px", background: form.name.trim() ? "linear-gradient(135deg, #a3b86c 0%, #7d9c3e 100%)" : "var(--surface-2)", color: form.name.trim() ? "#06090f" : "var(--text-muted)", border: "none", borderRadius: "var(--radius)", fontSize: "0.86rem", fontWeight: 700, cursor: form.name.trim() ? "pointer" : "not-allowed" }}>
            + Adicionar NPC
          </button>
          {mode === "random" && (
            <button onClick={regenerate} style={{ padding: "10px 18px", background: "transparent", color: "var(--text-muted)", border: "1px solid var(--border)", borderRadius: "var(--radius)", fontSize: "0.86rem", cursor: "pointer" }}>
              Aleatório
            </button>
          )}
        </div>
      ...
      {campaign.npcs.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginBottom: 4 }}>{campaign.npcs.length} NPC{campaign.npcs.length !== 1 ? "s" : ""}</p>
            ...
            <div key={npc.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", cursor: "pointer", gap: 10 }} onClick={() => setExpanded(expanded === npc.id ? null : npc.id)}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "var(--radius)", background: "rgba(125,156,62,0.14)", border: "1px solid rgba(125,156,62,0.32)", display: "flex", alignItems: "center", justifyContent: "center", color: "#a3b86c", fontSize: "0.9rem", flexShrink: 0 }}>
                    {npc.name?.[0] ?? "N"}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: "0.88rem", fontWeight: 700, color: "var(--text)" }}>{npc.name}</p>
                    <p style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
                      {[npc.occupation, npc.age ? `${npc.age} anos` : null].filter(Boolean).join(" · ")}
                      {(npc.hp || npc.san) && (
                    ...
                    {npc.mythosTies && npc.mythosTies !== "Nenhum (por enquanto)" && (
                      <span style={{ fontSize: "0.63rem", color: "#a3b86c", background: "rgba(125,156,62,0.14)", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius-xs)", padding: "1px 6px", display: "inline-block", marginTop: 2 }}>
                        Mythos
                      </span>
                    )}
                  </div>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  <button onClick={(e) => { e.stopPropagation(); if (addInitTarget === npc.id) { setAddInitTarget(null); return; } setAddInitTarget(npc.id); setAddInitValue(""); }} style={{ padding: "5px 10px", background: "rgba(125,156,62,0.14)", color: "#a3b86c", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius)", fontSize: "0.72rem", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                    ⚡ Ordem
                  </button>
              ...
              {addInitTarget === npc.id && (
                <div onClick={(e) => e.stopPropagation()} style={{ padding: "12px 16px", borderTop: "1px solid rgba(125,156,62,0.32)", background: "rgba(125,156,62,0.14)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button onClick={() => addToInitiative(npc)} style={{ padding: "8px 14px", background: "rgba(125,156,62,0.14)", color: "#a3b86c", border: "1px solid rgba(125,156,62,0.32)", borderRadius: "var(--radius)", fontSize: "0.82rem", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>+ Adicionar</button>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 6 }}>
                        {ATTR_LABELS.map(({ key, label }) => {
                          const val = npc[key] as number | null;
                          return (
                            <div key={key} style={{ textAlign: "center", padding: "6px 2px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                              ...
                              <p style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--text)" }}>{val ?? "—"}</p>
                            </div>
                          );
                        })}
                      </div>
                        ...
                        {npc.attacks.map((atk, i) => (
                          <div key={i} style={{ padding: "7px 10px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                            <p style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--text)", marginBottom: 2 }}>
                              {atk.name}
                              {atk.skill && <span style={{ color: "#a3b86c", fontWeight: 400 }}> · {atk.skill}</span>}
                              {atk.damage && <span style={{ color: "#7d9c3e", fontWeight: 400 }}> · {atk.damage}</span>}
                            </p>
