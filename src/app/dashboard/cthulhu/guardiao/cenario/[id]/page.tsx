...
const TABS: { id: Tab; label: string; icon: ReactNode }[] = [
  { id: "scenario",   label: "Cenário",       icon: icon("M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z") },
  ...
  { id: "insanity",   label: "Insanidade",    icon: icon("M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3 M12 17h.01 M12 22a10 10 0 1 0 0-20") },
  ...
  { id: "items",      label: "Artefatos",     icon: icon("M12 2 15 8l6 1-5 5 1 7-6-3-6 3 1-7-5-5 6-1z") },
...
export default function CthulhuCenarioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { status: cStatus, api } = useCthulhuCampaign(id);
  const [tab, setTab] = useState<Tab>("scenario");

  if (status === "unauthenticated") { router.push("/login"); return null; }

  if (status === "loading" || cStatus === "loading") {
    return <div style={{ minHeight: "100vh", background: "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Carregando...</p></div>;
  }
  ...
  if (cStatus === "notfound" || !api) {
    return (
      <div style={{ minHeight: "100vh", background: "transparent", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
        ...
        <Link href="/dashboard/cthulhu/guardiao" style={{ color: A, fontSize: "0.86rem", textDecoration: "none" }}>← Voltar ao Painel do Guardião</Link>
      </div>
    );
  }
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 24px 80px" }}>
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <span style={{ fontSize: "0.68rem", fontWeight: 700, color: A, background: ADIM, border: `1px solid ${ABORD}`, borderRadius: "var(--radius-xs)", padding: "2px 8px" }}>{ERA_LABEL[c.era] ?? c.era}</span>
          </div>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 28, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: 4, flexWrap: "wrap" }}>
          {TABS.map((t) => {
            ...
            return (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: "1 1 auto", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "9px 14px", background: active ? ADIM : "transparent", color: active ? A : "var(--text-muted)", border: active ? `1px solid ${ABORD}` : "1px solid transparent", borderRadius: "var(--radius-lg)", fontSize: "0.8rem", fontWeight: active ? 700 : 500, cursor: "pointer", transition: "all 0.18s", whiteSpace: "nowrap" }}>
                {t.icon}{t.label}
              </button>
            );
          })}
        ...
        {tab === "scenario"   && <CthulhuScenario api={api} />}
        {tab === "npc"        && <CthulhuNpcCreator api={api} />}
        {tab === "initiative" && <CthulhuInitiative api={api} />}
        {tab === "insanity"   && <CthulhuInsanity api={api} />}
        {tab === "clues"      && <CthulhuClues api={api} />}
        {tab === "clocks"     && <CthulhuClocks api={api} />}
        {tab === "items"      && <CthulhuArtefacts api={api} />}
        {tab === "bestiary"   && <CthulhuBestiary api={api} />}
