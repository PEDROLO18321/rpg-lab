      ...
      {adding && (
        <div style={{ background: "var(--surface)", border: `1px solid ${ABORD}`, borderRadius: "var(--radius-xl)", padding: "20px 24px" }}>
          ...
          <div className="cth-artefacts-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div style={{ gridColumn: "1 / -1" }}><Field label="Nome" style={labelStyle}><input style={inputStyle} value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} placeholder="Ex: Necronomicon, Amuleto de Cthulhu" autoFocus /></Field></div>
            ...
            <div><Field label="Vínculo à Sessão" style={labelStyle}><select style={selectStyle} value={newItem.sessionId ?? ""} onChange={(e) => setNewItem({ ...newItem, sessionId: e.target.value || null })}><option value="">Geral (cenário)</option>{sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}</select></Field></div>
            ...
            <div style={{ gridColumn: "1 / -1" }}><Field label="Descrição / Efeitos" style={labelStyle}><textarea style={{ ...inputStyle, resize: "vertical", minHeight: 80 }} value={newItem.description} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })} placeholder="Propriedades, lore, rituais, perigo para a sanidade..." /></Field></div>
          </div>
  ...
  return (
    <div style={{ padding: "0 20px 20px", borderTop: "1px solid var(--border)", paddingTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="cth-artefacts-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div><Field label="Vínculo" style={labelStyle}><select style={selectStyle} value={d.sessionId ?? ""} onChange={(e) => { const v = e.target.value || null; set({ sessionId: v }); commit({ sessionId: v }); }}><option value="">Geral (cenário)</option>{sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}</select></Field></div>
