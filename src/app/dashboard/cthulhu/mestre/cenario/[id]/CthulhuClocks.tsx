  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "end", padding: "16px 20px", background: "var(--surface)", border: `1px solid ${ABORD}`, borderRadius: "var(--radius-xl)" }}>
        <div style={{ flex: "2 1 200px" }}><Field label="Nome" style={lab}><input value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} placeholder="Ex: Ritual de invocação" style={{ ...ip, width: "100%" }} /></Field></div>
        ...
        <div><Field label="Tipo" style={lab}><select value={kind} onChange={(e) => setKind(e.target.value as CthulhuClock["kind"])} style={{ ...ip, cursor: "pointer" }}>{(["ameaca", "missao", "neutro"] as const).map((k) => <option key={k} value={k}>{KIND_LABEL[k]}</option>)}</select></Field></div>
