    ...
    <div style={{ position: "fixed", left: 0, top: "50%", transform: "translateY(-50%)", zIndex: 60, display: "flex", alignItems: "center" }}>
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <p style={{ fontSize: "0.72rem", fontWeight: 800, color: "var(--accent)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
              Dados do Mestre
            </p>
            <button
            ...
            {DICE_TYPES.map((die) => (
              <button
                ...
                style={{
                  ...
                  background: selectedDie === die ? "var(--accent-dim)" : "var(--surface-2)",
                  border: `1px solid ${selectedDie === die ? "var(--accent)" : "var(--border)"}`,
                  color: selectedDie === die ? "var(--accent-light)" : "var(--text-muted)",
                  cursor: "pointer",
                  fontSize: "0.66rem",
                  fontWeight: 800,
              ...
              {(["normal", "advantage", "disadvantage"] as const).map((mode) => (
                <button
                  ...
                  style={{
                    ...
                    background: advantage === mode ? "var(--accent-dim)" : "var(--surface-2)",
                    border: `1px solid ${advantage === mode ? "var(--accent)" : "var(--border)"}`,
                    color: advantage === mode ? "var(--accent-light)" : "var(--text-subtle)",
                    cursor: "pointer",
                    fontSize: "0.62rem",
                    fontWeight: 700,
                  }}
          ...
          <div className="dnd-dice-mod-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
              ...
              {(["sum", "max"] as const).map((mode) => (
                <button
                  ...
                  style={{
                    ...
                    padding: "7px 8px",
                    borderRadius: "var(--radius)",
                    background: pickMode === mode ? "var(--accent-dim)" : "var(--surface-2)",
                    border: `1px solid ${pickMode === mode ? "var(--accent)" : "var(--border)"}`,
                    color: pickMode === mode ? "var(--accent-light)" : "var(--text-muted)",
          ...
          <div style={{ minHeight: 86, marginTop: 14, padding: "14px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", textAlign: "center" }}>
            {lastRoll ? (
              <>
                ...
                <p key={lastRoll.id} style={{ fontFamily: "var(--font-cinzel), serif", fontSize: "2.5rem", fontWeight: 900, color: lastRoll.isCrit ? "var(--accent-light)" : lastRoll.isFumble ? "#ff6b6b" : "var(--text)", lineHeight: 1 }}>
                  {lastRoll.total}
                </p>
              </>
            ) : (
      ...
      <button
        ...
        style={{
          ...
          color: open ? "#06090f" : "var(--accent-light)",
          border: "1px solid var(--border-accent)",
          borderLeft: "none",
          cursor: "pointer",
