...
const KIND_COLOR: Record<DndClock["kind"], string> = { ameaca: "#e0524c", missao: "#7dd3a8", neutro: "#9aa0a6" };
const KIND_LABEL: Record<DndClock["kind"], string> = { ameaca: "Ameaça", missao: "Missão", neutro: "Neutro" };
...
export function CampaignClocks({ api }: { api: DndApi }) {
  ...
  function tick(c: DndClock, delta: number) { api.editChild("clocks", c.id, { filled: Math.max(0, Math.min(c.segments, c.filled + delta)) }); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div>
        ...
        <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginTop: 4 }}>Acompanhe ameaças, rituais ou objetivos avançando segmento a segmento.</p>
      </div>
