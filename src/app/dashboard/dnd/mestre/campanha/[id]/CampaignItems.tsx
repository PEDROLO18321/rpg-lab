...
import type { DndCampaignItem } from "@/lib/dnd/dndCampaignClient";
...
type ItemType = DndCampaignItem["type"];
type ItemRarity = DndCampaignItem["rarity"];

const TYPE_OPTIONS: { value: ItemType; label: string }[] = [
  { value: "arma", label: "Arma" }, { value: "armadura", label: "Armadura" },
  { value: "magia", label: "Item Mágico" }, { value: "consumível", label: "Consumível" }, { value: "misc", label: "Misc" },
];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {adding && (
        <div style={{ background: "var(--surface)", border: `1px solid ${ACCENT_BORD}`, borderRadius: "var(--radius-xl)", padding: "20px 24px" }}>
          ...
          <div className="dnd-items-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div style={{ gridColumn: "1 / -1" }}><Field label="Nome" style={labelStyle}><input style={inputStyle} value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} placeholder="Nome do item" autoFocus /></Field></div>
            <div><Field label="Tipo" style={labelStyle}><select style={selectStyle} value={newItem.type} onChange={(e) => setNewItem({ ...newItem, type: e.target.value as ItemType })}>{TYPE_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select></Field></div>
            <div><Field label="Raridade" style={labelStyle}><select style={selectStyle} value={newItem.rarity} onChange={(e) => setNewItem({ ...newItem, rarity: e.target.value as ItemRarity })}>{RARITY_OPTIONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</select></Field></div>
            <div style={{ gridColumn: "1 / -1" }}><Field label="Vínculo" style={labelStyle}><select style={selectStyle} value={newItem.sessionId ?? ""} onChange={(e) => setNewItem({ ...newItem, sessionId: e.target.value || null })}><option value="">Geral (campanha)</option>{sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}</select></Field></div>
            <div style={{ gridColumn: "1 / -1" }}><Field label="Descrição" style={labelStyle}><textarea style={{ ...inputStyle, resize: "vertical", minHeight: 70 }} value={newItem.description} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })} placeholder="Propriedades, lore, efeitos..." /></Field></div>
          </div>
...
function ItemEditor({ api, item, sessions, onDeleted }: { api: DndApi; item: DndCampaignItem; sessions: DndApi["campaign"]["sessions"]; onDeleted: () => void }) {
  ...
  return (
    <div style={{ padding: "0 20px 20px", borderTop: "1px solid var(--border)", paddingTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="dnd-items-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Nome" style={labelStyle}><input style={inputStyle} value={d.name} onChange={(e) => set({ name: e.target.value })} onBlur={(e) => commit({ name: e.target.value })} /></Field></div>
        <div><Field label="Tipo" style={labelStyle}><select style={selectStyle} value={d.type} onChange={(e) => { set({ type: e.target.value as ItemType }); commit({ type: e.target.value as ItemType }); }}>{TYPE_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}</select></Field></div>
        <div><Field label="Raridade" style={labelStyle}><select style={selectStyle} value={d.rarity} onChange={(e) => { set({ rarity: e.target.value as ItemRarity }); commit({ rarity: e.target.value as ItemRarity }); }}>{RARITY_OPTIONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}</select></Field></div>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Vínculo à Sessão" style={labelStyle}><select style={selectStyle} value={d.sessionId ?? ""} onChange={(e) => { const v = e.target.value || null; set({ sessionId: v }); commit({ sessionId: v }); }}><option value="">Geral (campanha)</option>{sessions.map((s) => <option key={s.id} value={s.id}>Sessão {s.number}{s.name ? ` · ${s.name}` : ""}</option>)}</select></Field></div>
        <div style={{ gridColumn: "1 / -1" }}><Field label="Descrição" style={labelStyle}><textarea style={{ ...inputStyle, resize: "vertical", minHeight: 80 }} value={d.description} onChange={(e) => set({ description: e.target.value })} onBlur={(e) => commit({ description: e.target.value })} /></Field></div>
      </div>
