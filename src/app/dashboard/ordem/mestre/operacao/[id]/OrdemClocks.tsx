...
const KIND_COLOR: Record<OrdemClock["kind"], string> = {
  ameaca: "#e0524c", missao: "#7dd3a8", neutro: "#9aa0a6",
};
const KIND_LABEL: Record<OrdemClock["kind"], string> = {
  ameaca: "Ameaça", missao: "Missão", neutro: "Neutro",
};
...
function ClockDial({ segments, filled, color, size = 84 }: { segments: number; filled: number; color: string; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {slices.map((s, i) => (
        <path key={i} d={s.d} fill={s.on ? color : "transparent"} fillOpacity={s.on ? 0.85 : 0} stroke="rgba(255,255,255,0.35)" strokeWidth="1.2" />
      ))}
...
export function OrdemClocks({ api }: { api: OperacaoApi }) {
  ...
  async function add() {
    if (!name.trim()) return;
    await api.addChild("clocks", { name: name.trim(), segments, filled: 0, kind });
    setName("");
  }

  function tick(c: OrdemClock, delta: number) {
    const filled = Math.max(0, Math.min(c.segments, c.filled + delta));
    api.editChild("clocks", c.id, { filled });
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div>
        <h2 style={{ fontFamily: "var(--font-cinzel), serif", fontSize: "1rem", fontWeight: 700, color: "var(--text)", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: AL }}>⏳</span> Relógios de Tensão
        </h2>
        <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginTop: 4 }}>
          Marque o avanço de uma ameaça, ritual ou objetivo. Preencha segmentos conforme a tensão sobe.
        </p>
      </div>
      ...
      {/* Grid */}
      {clocks.length === 0 ? (
        <p style={{ fontSize: "0.84rem", color: "var(--text-subtle)", textAlign: "center", padding: "32px 0" }}>Nenhum relógio. Crie um para acompanhar a tensão.</p>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 }}>
          {clocks.map((c: OrdemClock) => {
