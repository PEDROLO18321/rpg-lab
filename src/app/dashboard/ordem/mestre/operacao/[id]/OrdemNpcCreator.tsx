...
import { type OrdemCampaign, type OrdemNPC, type OrdemNPCAttack } from "@/lib/ordem/ordemCampaignStorage";
...
const PERSONALITIES = [
  "Desconfiado e fechado, esconde o que sabe",
  "Curioso demais para o próprio bem",
  "Aterrorizado pelo que viu mas não esquece",
  "Finge normalidade enquanto se despedaça por dentro",
  "Fanático por uma entidade do Outro Lado",
  "Calmo sob pressão, corajoso",
  "Nervoso e propenso a surtos",
  "Pragmático, faz o necessário",
  "Obcecado por uma teoria sobre o Paranormal",
  "Manipulador, busca poder",
];
...
const ATTR_LABELS: { key: keyof Pick<OrdemNPC, "agi"|"forc"|"int"|"pre"|"vig">; label: string }[] = [
  { key: "agi", label: "AGI" }, { key: "forc", label: "FOR" }, { key: "int", label: "INT" },
...
function emptyNpc(): Omit<OrdemNPC, "id"> {
  return { name: "", role: "", age: null, gender: "", affiliation: "", description: "", personality: "", paranormalTies: "", notes: "", agi: null, forc: null, int: null, pre: null, vig: null, pv: null, pe: null, san: null, defense: null, attacks: [] };
}

function randomNpc(): Omit<OrdemNPC, "id"> {
  const vig = randBetween(1, 3);
  const agi = randBetween(1, 3);
  const pv = 10 + vig * 5 + randBetween(0, 5);
  return {
    name: randomOrdemName(),
    role: rand(ROLES),
    age: randBetween(18, 65),
    gender: rand(["Masculino", "Feminino", "Não-binário"]),
    affiliation: rand(AFFILIATIONS),
    ...
    notes: "",
    agi,
    forc: randBetween(1, 3),
    int: randBetween(1, 3),
    pre: randBetween(1, 3),
    vig,
    pv,
    pe: 2 + randBetween(0, 4),
    san: 10 + randBetween(0, 10),
    defense: 10 + agi,
...
export function OrdemNpcCreator({ campaign, onChange }: Props) {
  const [mode, setMode] = useState<"random" | "manual">("random");
  const [form, setForm] = useState<Omit<OrdemNPC, "id">>(randomNpc());
  ...
  function saveNpc() {
    ...
    const npc: OrdemNPC = { ...form, id: crypto.randomUUID() };
  ...
  function addAttack() {
    ...
    setNewAtk({ name: "", test: "", damage: "", description: "" });
  }
  ...
  function addToInitiative(npc: OrdemNPC) {
    const init = addInitValue !== "" ? Number(addInitValue) : (npc.agi ?? 1) + Math.floor(Math.random() * 20) + 1;
    onChange({
      ...campaign,
      combatants: [
        ...campaign.combatants,
        { id: crypto.randomUUID(), name: npc.name, init, pv: npc.pv, maxPv: npc.pv, pe: npc.pe, maxPe: npc.pe, san: npc.san, maxSan: npc.san, isPlayer: false },
      ],
    });
  ...
  const inputStyle: React.CSSProperties = {
    padding: "8px 10px", background: "var(--surface-2)", border: `1px solid ${AB}`, borderRadius: "var(--radius)",
    color: "var(--text)", fontSize: "0.84rem", width: "100%", boxSizing: "border-box",
  };
  ...
  const sectionLabel: React.CSSProperties = {
    fontSize: "0.7rem", fontWeight: 700, color: A, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10,
  };

  function textField(label: string, key: keyof typeof form, opts?: { list?: string[] }) {
    return (
  ...
  function numField(label: string, key: keyof typeof form, max = 99) {
    return (
    ...
    <div>
      ...
      <div style={{ padding: "24px", background: "var(--surface)", border: `1px solid ${AB}`, borderRadius: "var(--radius-xl)", marginBottom: 28 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          {textField("Ligação com o Paranormal", "paranormalTies")}
        </div>
        ...
        <div style={{ borderTop: `1px solid ${AB}`, paddingTop: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            ...
            <button onClick={addAttack} disabled={!newAtk.name.trim()} style={{ padding: "8px 14px", background: newAtk.name.trim() ? AD : "var(--surface-2)", color: newAtk.name.trim() ? AL : "var(--text-subtle)", border: `1px solid ${AB}`, borderRadius: "var(--radius)", fontSize: "0.82rem", fontWeight: 700, cursor: newAtk.name.trim() ? "pointer" : "not-allowed", whiteSpace: "nowrap" }}>
              + Ataque
            </button>
          </div>
        </div>
            ...
            <div key={npc.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", cursor: "pointer", gap: 10 }} onClick={() => setExpanded(expanded === npc.id ? null : npc.id)}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "var(--radius)", background: AD, border: `1px solid ${AB}`, display: "flex", alignItems: "center", justifyContent: "center", color: AL, fontSize: "0.9rem", flexShrink: 0 }}>
                    {npc.name?.[0] ?? "N"}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    ...
                    {npc.paranormalTies && npc.paranormalTies !== "Nenhuma (por enquanto)" && (
                      <span style={{ fontSize: "0.63rem", color: "#f87171", background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: "var(--radius-xs)", padding: "1px 6px", display: "inline-block", marginTop: 2 }}>
                        Paranormal
                      </span>
                    )}
                  </div>
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  <button onClick={(e) => { e.stopPropagation(); if (addInitTarget === npc.id) { setAddInitTarget(null); return; } setAddInitTarget(npc.id); setAddInitValue(""); }} style={{ padding: "5px 10px", background: AD, color: AL, border: `1px solid ${AB}`, borderRadius: "var(--radius)", fontSize: "0.72rem", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                    ⚡ Ordem
                  </button>
                        ...
                        {npc.attacks.map((atk, i) => (
                          <div key={i} style={{ padding: "7px 10px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--border)" }}>
                            <p style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--text)", marginBottom: 2 }}>
                              {atk.name}
                              {atk.test && <span style={{ color: AL, fontWeight: 400 }}> · {atk.test}</span>}
                              {atk.damage && <span style={{ color: "var(--text-muted)", fontWeight: 400 }}> · {atk.damage}</span>}
                            </p>
