...
const TYPE_COLOR: Record<ItemType, string> = {
  arma: "#fbbf24", protecao: "#60a5fa", "item-paranormal": "#f87171",
  equipamento: "#a3b86c", ritual: "#a78bfa", misc: "var(--text-muted)",
};
...
function blankItem() {
  return { name: "", description: "", type: "misc" as ItemType, paranormal: false, sessionId: null as string | null };
}
            ...
            return (
              <div key={item.id} style={{ background: "var(--surface)", border: `1px solid ${isOpen ? AB : "var(--border)"}`, borderRadius: "var(--radius-xl)", overflow: "hidden", transition: "border-color 0.2s" }}>
                <button onClick={() => setExpandedId(isOpen ? null : item.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "13px 20px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
                  ...
                  {item.paranormal && <span style={{ fontSize: "0.63rem", fontWeight: 700, color: "#f87171", background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.25)", borderRadius: "var(--radius-xs)", padding: "2px 7px", flexShrink: 0 }}>Paranormal</span>}
                  ...
                  <span style={{ fontSize: "0.65rem", fontWeight: 700, color: AL, background: AD, border: `1px solid ${AB}`, borderRadius: "var(--radius-xs)", padding: "2px 7px", flexShrink: 0 }}>{sessionName(item.sessionId)}</span>
...
function ItemEditor({ api, item, sessions, onDeleted }: {
  ...
  const commit = (p: Partial<OrdemItem>) => api.editChild("items", item.id, p);
  ...
  return (
    <div style={{ padding: "0 20px 20px", borderTop: "1px solid var(--border)", paddingTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="op-items-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "0.78rem", color: "var(--text-muted)", cursor: "pointer" }}>
            <input type="checkbox" checked={d.paranormal} onChange={(e) => { set({ paranormal: e.target.checked }); commit({ paranormal: e.target.checked }); }} />
            Item Paranormal
          </label>
        </div>
        <div style={{ gridColumn: "1 / -1" }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button onClick={() => { api.removeChild("items", item.id); onDeleted(); }} style={{ padding: "7px 16px", background: "rgba(220,60,60,0.1)", border: "1px solid rgba(220,60,60,0.3)", borderRadius: "var(--radius)", color: "#e06c6c", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer" }}>
          Excluir Item
        </button>
      </div>
    </div>
