    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {adding && (
        <div style={{ background: "var(--surface)", border: `1px solid ${ACCENT_BORD}`, borderRadius: "var(--radius-xl)", padding: "20px 24px" }}>
          ...
          <div className="tm-items-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div style={{ gridColumn: "1 / -1" }}><Field label="Nome" style={labelStyle}><input style={inputStyle} value={newItem.name} onChange={(e) => setNewItem({ ...newItem, name: e.target.value })} placeholder="Ex: Espada Justiceira de Khalmyr" autoFocus /></Field></div>
...
function ItemEditor({ api, item, sessions, onDeleted }: { api: TormentaApi; item: TormentaCampaignItem; sessions: TormentaApi["campaign"]["sessions"]; onDeleted: () => void }) {
  ...
  return (
    <div style={{ padding: "0 20px 20px", borderTop: "1px solid var(--border)", paddingTop: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div className="tm-items-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
