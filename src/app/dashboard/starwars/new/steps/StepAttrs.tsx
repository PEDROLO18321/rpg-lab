...
import { ATTR_KEYS, ATTR_LABEL, ATTR_ABBR, ATTR_CREATION_POINTS, ATTR_CREATION_MAX_PER_ATTR, attrPointsRemaining, attrPointsSpent, type StarWarsAttrs } from "@/lib/starwars/data";
...
export function StepAttrs({ attrBases, attrsFinal, speciesId, onChange }: Props) {
  ...
  function set(k: keyof StarWarsAttrs, delta: number) {
    ...
    if (value < 0) return;
    if (delta > 0 && (remaining <= 0 || value > ATTR_CREATION_MAX_PER_ATTR)) return;
    onChange({ attrBases: { ...attrBases, [k]: value } });
  }

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 26 }}>
      <SectionTitle eyebrow="Etapa 4 de 8" title="Atributos" desc={`Distribua ${ATTR_CREATION_POINTS} pontos entre os 6 atributos, no máximo ${ATTR_CREATION_MAX_PER_ATTR} num só. O valor Final já inclui o modificador de ${species?.name ?? "espécie"} — pode ficar negativo. Regra de rolagem: valor 0 rola 2d20 e usa o menor; valor negativo rola (2 + |valor|) d20 e usa o menor; valor 1 rola 1d20 normal; valor 2 ou mais rola N d20 (N = valor) e usa o maior.`} />
        ...
        {ATTR_KEYS.map((k) => {
          ...
          return (
            <div key={k} className="sw-attr-row" style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 18px", background: "rgba(255,255,255,0.015)", border: "1px solid var(--border)" }}>
              ...
              <button onClick={() => set(k, 1)} disabled={remaining <= 0 || attrBases[k] >= ATTR_CREATION_MAX_PER_ATTR}
                style={{ width: 28, height: 28, borderRadius: "50%", border: `1px solid ${SW.accentBord}`, background: "rgba(255,255,255,0.03)", color: SW.accentLight, cursor: "pointer", fontWeight: 700 }}>+</button>
