      ...
      <section style={{ background: SW.panel, border: `1px solid ${ACCENT_BORD}`, borderRadius: "var(--radius-xl)", padding: "24px 28px", boxShadow: `0 0 24px ${SW.glow}, inset 0 1px 0 rgba(255,255,255,0.03)` }}>
        ...
        <div className="sw-story-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          <div>
            ...
            <textarea style={{ ...areaStyle, minHeight: 72 }} value={draft.objective} onChange={(e) => setField({ objective: e.target.value })} onBlur={(e) => commit({ objective: e.target.value })} placeholder="O que o grupo deve alcançar?" />
          </div>
          <div>
            <label style={labelStyle}>Propósito / Tema</label>
            <textarea style={{ ...areaStyle, minHeight: 72 }} value={draft.purpose} onChange={(e) => setField({ purpose: e.target.value })} onBlur={(e) => commit({ purpose: e.target.value })} placeholder="Exploração, redenção, sobrevivência..." />
          </div>
          <div>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            <textarea style={{ ...areaStyle, minHeight: 120 }} value={draft.generalHistory} onChange={(e) => setField({ generalHistory: e.target.value })} onBlur={(e) => commit({ generalHistory: e.target.value })} placeholder="Contexto da galáxia, eventos passados, facções, segredos..." />
          </div>
        </div>
      </section>
