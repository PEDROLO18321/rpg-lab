...
interface ShareData {
  name: string;
  cthulhuStory: { currentArc: string } | null;
  cthulhuCombatants: { id: string; name: string; dex: number; isPlayer: boolean; conditions: string }[];
  cthulhuClocks: { id: string; name: string; segments: number; filled: number; kind: string }[];
  cthulhuClues: { id: string; title: string; content: string; source: string }[];
}
...
export default function CthulhuPlayerViewPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    let alive = true;
    const load = () => fetch(`/api/cthulhu/campaigns/share/${code}`)
  ...
  if (status === "loading") return <Center>Carregando cenário...</Center>;
  if (status === "error" || !data) return <Center>Cenário não encontrado.</Center>;

  return (
    <div style={{ minHeight: "100vh", background: "transparent", maxWidth: 900, margin: "0 auto", padding: "40px 24px 80px" }}>
      <span style={{ fontSize: "0.72rem", fontWeight: 700, color: "#a3b86c", letterSpacing: "0.1em", textTransform: "uppercase" }}>Call of Cthulhu · Tela do Investigador</span>
      ...
      {data.cthulhuStory?.currentArc && (
        <section style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-xl)", padding: "20px 24px", marginBottom: 20 }}>
          <p style={{ fontSize: "0.9rem", color: "var(--text-muted)", lineHeight: 1.7 }}>{data.cthulhuStory.currentArc}</p>
        </section>
      )}

      {data.cthulhuCombatants.length > 0 && (
        <Section title="Ordem de Ação (DEX)">
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {data.cthulhuCombatants.map((c) => {
              let conds: string[] = [];
              ...
              return (
                <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--radius)" }}>
                  <span style={{ minWidth: 28, textAlign: "center", fontWeight: 800, color: "#a3b86c", fontFamily: "var(--font-cinzel), serif" }}>{c.dex}</span>
                  <span style={{ flex: 1, fontWeight: 700, color: c.isPlayer ? "#63b3ed" : "var(--text)" }}>{c.name}</span>
                  {conds.length > 0 && <span style={{ fontSize: "0.7rem", color: "#fbbf24" }}>{conds.length} estado(s)</span>}
                </div>
              );
            })}
          </div>
      ...
      {data.cthulhuClocks.length > 0 && (
        <Section title="Relógios">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12 }}>
            {data.cthulhuClocks.map((c) => {
      ...
      {data.cthulhuClues.length > 0 && (
        <Section title="Pistas Reveladas">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {data.cthulhuClues.map((c) => (
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section style={{ marginBottom: 24 }}>
      <h2 style={{ fontFamily: "var(--font-cinzel), serif", fontSize: "0.92rem", fontWeight: 700, color: "#a3b86c", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ width: 6, height: 6, borderRadius: "50%", background: "rgba(125,156,62,0.32)" }} />{title}
      </h2>
      {children}
    </section>
  );
